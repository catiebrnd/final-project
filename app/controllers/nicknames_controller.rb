class NicknamesController < ApplicationController
  ...
  def index
    if params[:person]
      @person = BAMBOO_CLIENT.employee.all.find { |employee| employee['displayName'] == params[:person][:name] }
      offset = rand(Nickname.count)
      @nickname = Nickname.offset(offset).first
    end
  end
  ...
  private
    ...
    def nickname_params
      params.require(:nickname).permit(:first_name, :last_name, :employee_id)
    end
end
